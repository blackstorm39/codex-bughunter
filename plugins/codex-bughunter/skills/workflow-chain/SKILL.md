---
name: workflow-chain
description: "Explicit Codex workflow. Build an exploit chain — given bug A, finds B and C to combine for higher severity and payout. Knows common chain patterns: IDOR→ATO, SSRF→cloud metadata, XSS→ATO, open redirect→OAuth theft, S3→bundle→secret→OAuth. Invoke only as $codex-bughunter:workflow-chain."
user-invocable: true
...
```
$codex-bughunter:workflow-chain
```
