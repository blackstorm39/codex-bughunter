...
| **Codex distribution** | 83 domain skills + 15 explicit workflows | This fork |
...
- **`hunt <target>` scaffold** — Engagement-folder scaffolding creates a local workspace with `AGENTS.md`, `scope.md`, `findings/`, `evidence/`, `submissions.txt`, `notes.md`, and a defensive `.gitignore`. Ships as `scripts/hunt.sh` and `scripts/hunt.ps1`.
- **Codex packaging** — Local marketplace metadata installs 83 domain skills and 15 explicit Codex workflow skills. `scripts/install.sh` and `scripts/install.ps1` wrap the native Codex plugin commands.
- **Autopilot ledger (`engine/memory.py`) + the `workflow-remember`, `workflow-memory-gc`, and `workflow-pickup` skills** — Original design and implementation (cross-engagement capture + skip-decision for the engine hunt loop). Not derived from any external memory implementation.
