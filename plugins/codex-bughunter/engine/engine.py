...
State is persisted after every step, so a run is auditable and resumable.
  ...
  python engine/engine.py --scope my-engagement.json --max-hunts 8              # live (needs Codex auth)
...
try:
    ...
    from . import memory
    from . import osint as O
    from . import recon as R
    from . import skill_map as SM
except ImportError:  # Direct-script compatibility.
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    from scope import Scope            # type: ignore  # noqa: E402
    from state import Engagement       # type: ignore  # noqa: E402
    import agent as A                  # type: ignore  # noqa: E402
    import memory                      # type: ignore  # noqa: E402
    import osint as O                  # type: ignore  # noqa: E402
    import recon as R                  # type: ignore  # noqa: E402
    import skill_map as SM             # type: ignore  # noqa: E402
...
CLASS_WEIGHT = {"rce": 100, "sqli": 90, "ssrf": 85, "auth-bypass": 85, "idor": 80,
                ...
                "info-leak": 30}
# OSINT (subdomain/asset enum) is a separate concern — not in the engine flow.
...
class Engine:
    def __init__(self, scope_path, base, model, max_hunts, max_turns, timeout, mock=False,
        ...
        invalid_seeds = [(seed, self.scope.reject_reason(seed)) for seed in self.scope.seeds
                         if self.scope.reject_reason(seed) is not None]
        if invalid_seeds:
            details = "; ".join(f"{seed}: {reason}" for seed, reason in invalid_seeds)
            raise ValueError(f"scope contains out-of-scope seed(s): {details}")
    ...
    def _expand_scope(self):
        ...
        a separate concern; this just ingests its result when you want one tool to do
    ...
    def recon(self):
        ...
        api = R.openapi_recon(
            self.scope, log=self.eng.log, allow_intrusive=self.allow_intrusive
        )                                                            # schema + authorized auth-sweep
    ...
    def hunt(self):
        ...
        for it, f in self._run_parallel(self._hunt_agent, todo):
            if f and f.get("rate_limited"):
                self.eng.log("hunt: ⚠ Codex usage limit hit — remaining results may be partial")
    ...
    def _hunt_agent(self, it):
        ...
        task = (self._roe() + "\n"
                ...
                f"within the rules of engagement above. Apply these installed Codex skills when available: "
                f"{', '.join('$codex-bughunter:' + skill for skill in skills)}. "
                ...
                f'{{"vulnerable":true|false,"severity":"none|low|medium|high|critical","evidence":"<what proves it>",'
        ...
        r = A.run_agent(task, model=self.model, max_turns=self.max_turns,
                        timeout=self.timeout, schema="hunt", cwd=self.eng.dir)
        if r.get("error"):
    ...
    def _validate_agent(self, c):
        ...
        task = (self._roe() + "\n"
                ...
                f"Return one JSON object matching this shape: "
                f'{{"real":true|false,"severity":"none|low|medium|high|critical","reason":"<why, incl. any scope/intrusive caveat>"}}.')
        r = A.run_agent(task, model=self.model, max_turns=self.max_turns,
                        timeout=self.timeout, schema="validate", cwd=self.eng.dir)
        if r.get("error"):
...
def main():
    ...
    ap.add_argument("--model", default=None,
                    help="optional Codex model override; default uses the configured model")
    ap.add_argument("--max-hunts", type=int, default=10)
    ap.add_argument("--max-turns", type=int, default=40,
                    help="deprecated compatibility option; timeout bounds Codex execution")
