...
# $HOME\Targets\ with AGENTS.md, scope.md, submissions tracker, findings
...
4. **Validate** — run `$codex-bughunter:workflow-triage` on every lead BEFORE drafting a report.
...
- `$codex-bughunter:workflow-scope <asset>` — verify a specific asset is in scope
- `$codex-bughunter:workflow-triage` — quick 7-Question Gate on a finding
- `$codex-bughunter:workflow-validate` — full 4-gate finding validator
- `$codex-bughunter:workflow-report` — draft a submission-ready report
- `$codex-bughunter:workflow-remember` — log a finding to hunt memory
'@
    $agentsMd = "# Engagement: $Target`r`n`r`n**Target:** $Target`r`n**Started:** $today`r`n" + $agentsBody
    Write-HuntFile (Join-Path $dir 'AGENTS.md') $agentsMd

    # ============== scope.md ==============
    $scopeBody = @'

> Parse this from the program page (Bugcrowd / HackerOne / etc.) before
> doing any active testing. `$codex-bughunter:workflow-scope <asset>` can verify individual assets.
...
#
# Use `$codex-bughunter:workflow-remember` after each submission to append the new ID and link
...
'@
    ...
    # ============== confirmation ==============
    Write-Host "Initialized $dir with AGENTS.md and engagement template."
    Write-Host "cd $dir to start hacking."
    Write-Host ""
    Write-Host "Files created:"
    Write-Host "  AGENTS.md           - Codex engagement context"
