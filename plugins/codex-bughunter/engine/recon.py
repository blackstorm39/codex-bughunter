...
stdlib-only fallback; uses `requests` if available. Redirects are never followed:
the caller must scope-check every new destination before requesting it.
"""
import re
...
try:
    ...
    def _get(url, timeout=20):
        try:
            r = _SESS.get(url, timeout=timeout, allow_redirects=False)
            try:
...
def spa_recon(scope, seeds=None, max_js=20, interesting_only=True, log=print):
    ...
    for seed in (seeds or scope.seeds):
        ...
        if not scope.in_scope_host(final):
            log(f"recon: response destination {final} not in scope — skip"); continue
...
def _gau_urls(scope, log):
    ...
    for s in scope.seeds:
        if not scope.in_scope_host(s):
            log(f"params: seed {s} not in scope — skip")
            continue
...
def _katana_urls(scope, log, depth=2, timeout=150):
    ...
    for seed in scope.seeds:
        if not scope.in_scope_host(seed):
            log(f"params: seed {seed} not in scope — skip")
            continue
        try:
...
def _probe(url, timeout=12):
    ...
    import subprocess
    try:
        out = subprocess.run(["curl", "-s", "-m", str(timeout), "-w", "\\n__ST__%{http_code}", url],
...
def auth_enforcement_sweep(spec, base, log=print, junk="zzz-pentest-nonexistent-00000",
    ...
    declared != enforced). The default is GET-only. State-changing methods are probed only after
    the operator explicitly enables intrusive testing. 2xx/422 unauth = NOT enforced."""
    import urllib.request
    import urllib.error
    import time
    class _NoRedirectHandler(urllib.request.HTTPRedirectHandler):
    ...
    allowed_methods = ("get", "post", "put", "patch", "delete") if allow_intrusive else ("get",)
...
def platform_recon(scope, log=print):
    ...
    for seed in scope.seeds:
        if not scope.in_scope_host(seed):
            log(f"platform: seed {seed} not in scope — skip")
            continue
            ...
            if "firebase/init" in wk and b.startswith("{"):
                ...
                try:
                    ...
                    for k in ("authorization_endpoint", "token_endpoint", "registration_endpoint"):
                        if meta.get(k) and scope.in_scope_host(meta[k]):
...
def openapi_recon(scope, log=print, allow_intrusive=False):
    ...
    for seed in scope.seeds:
        if not scope.in_scope_host(seed):
            log(f"openapi: seed {seed} not in scope — skip")
            continue
        host = seed.rstrip("/")
        if "://" not in host:
            host = "https://" + host
        for op_path in OPENAPI_PATHS:
            ...
            items += auth_enforcement_sweep(
                spec, host, log, allow_intrusive=allow_intrusive
            )
            break
    return items
