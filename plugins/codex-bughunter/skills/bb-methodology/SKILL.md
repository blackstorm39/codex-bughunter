...
**Command**: `$codex-bughunter:workflow-recon target.com`
...
**Pre-report gate:**
```
Run $codex-bughunter:workflow-validate (7-Question Gate)
+-- All 7 pass? -> Write report
+-- Any fail? -> KILL the finding. Don't waste time.
+-- Borderline? -> Run $codex-bughunter:workflow-triage for quick go/no-go
```
...
**Report:**
```
Run $codex-bughunter:workflow-report
...
- [ ] Record finding with `$codex-bughunter:workflow-remember` for hunt memory
...
- [ ] Log findings with `$codex-bughunter:workflow-remember`
...
- **`triage-validation`** — When a finding completes Phase 4 and is about to be written up. Workflow primitive: Phase 5 explicitly calls `$codex-bughunter:workflow-validate` (the 7-Question Gate); only findings that pass all 7 questions get handed off to `report-writing`.
