...
def probe(host, timeout=12):
    ...
    for scheme in ("https", "http"):
        ...
        os.close(fd)
        try:
            out = _run(["curl", "-s", "-m", str(timeout), "-D", "-", "-o", body_path, url], timeout + 6)
            ...
            try:
                with open(body_path, encoding="utf-8", errors="replace") as body_file:
                    body = body_file.read()
            except Exception:
                body = ""
        finally:
...
def osint(scope, log=print, max_probe=80):
    ...
    for s in scope.seeds:
        if not scope.in_scope_host(s):
            log(f"osint: seed {s} not in scope — skip")
            continue
