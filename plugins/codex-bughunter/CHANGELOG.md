...
### Fixed
- Synced the live plugin, package, CLI producer, docs, and banner to `2.1.1`.
- Replaced stale fork-owned legacy release URLs with the canonical
  `https://git.ot9.me/Adminx/codex-bughunter` release surface while preserving
  upstream GitHub attribution links.
- Tightened both installers to parse Codex JSON, validate the exact
  `codex-bughunter-local` root and `codex-bughunter@codex-bughunter-local`
  source, and refresh the plugin with `codex plugin add ... --json` on repeat runs.
- Expanded install docs for local checkout, pinned Git tag, branch-tracking
  updates, and pinned-tag migration or rollback without removing engagement or
  memory state.
- Added contract coverage for marketplace policy/auth/category, canonical URLs,
  logo presence, default-prompt limits, version parity, and installer refresh
  behavior without touching real Codex state.

## [2.1.0 Codex fork] - 2026-09-01

### Codex fork
- Packaged 83 domain skills and 15 converted workflows as a native Codex plugin.
- Replaced Claude command dispatch with sandboxed `codex exec` and JSON Schema results.
- Added Codex-only installers, portable memory locking, CLI entry points, and opt-in Burp MCP setup.
- Made scope exact-host/wildcard-explicit and fail-closed, disabled automatic redirects,
  constrained Katana to the authorized FQDN, and kept active write methods opt-in.
- Made memory deduplication/maintenance atomic and preserved existing MCP entries by default.
- Preserved upstream methodology, public-report corpus, attribution, and licenses.
