---
name: workflow-hunt
description: "Explicit Codex workflow. Active vulnerability hunting. Two-track dispatcher — asks Red Team vs WAPT, hands off to hunt-dispatch skill and sibling commands. Invoke only as $codex-bughunter:workflow-hunt."
user-invocable: true
...
This workflow is explicit-only. Treat text after `$codex-bughunter:workflow-hunt` as its arguments. Resolve bundled helpers relative to this installed skill (plugin root is two directories above `skills/workflow-hunt`), while writing engagement artifacts only in the user's current workspace. Confirm authorization and enforce recorded scope before any active request.

slim two-track dispatcher. one mode question, one branch, delegate.

**engagement frame — state once, then proceed.** invoking `$codex-bughunter:workflow-hunt` asserts the operator holds written
...
*.target.com             wildcard — $codex-bughunter:workflow-recon <base> first, then hunt each live host
...
wildcard handler: if `$TARGET` begins with `*.`, strip prefix and invoke `$codex-bughunter:workflow-recon <base>` before continuing.
...
`$codex-bughunter:workflow-scope` (deterministic, deny-wins) and the `triage-validation` 7-Question Gate before
...
```
mode: redteam
severity gate: critical / high  ·  medium only if it chains via $codex-bughunter:workflow-chain
...
```
before any HTTP touch    →  $codex-bughunter:workflow-scope     (mandatory pre-flight)
recon empty | wildcard   →  $codex-bughunter:workflow-recon <target>
5+ live hosts surfaced   →  $codex-bughunter:workflow-surface   (P1/P2/Kill list)
confirmed finding        →  $codex-bughunter:workflow-chain     (A→B table lives here, NOT in $codex-bughunter:workflow-hunt)
before any report        →  $codex-bughunter:workflow-validate  (7-Question Gate)
findings ready           →  $codex-bughunter:workflow-report    (suggest, never auto)
session end              →  $codex-bughunter:workflow-remember  (silent)
```

## step 4 — active testing

hand off to the loaded `hunt-*` skills. each skill has its own probes, payloads, validation. do not duplicate that logic here. on every confirmed finding, invoke `$codex-bughunter:workflow-chain` to check the A→B signal table.
...
at session end, invoke `$codex-bughunter:workflow-remember` silently (non-fatal).
