---
name: hunt-dispatch
description: "Skill-set loader for the explicit workflow-hunt orchestrator. Fingerprints an authorized target, selects the platform attack skills, and loads the Red Team or WAPT set after mode selection. Not for direct user invocation."
sources: community
report_count: 0
---
...
skill-set loader for `$codex-bughunter:workflow-hunt`. one concept (which skills to load), one place.
...
- **authorized and scope-bounded.** the operator asserted written authorization for the named scope
  when they invoked `$codex-bughunter:workflow-hunt`. testing stays inside it. an out-of-scope host ends the run — it does not
  widen it.
...
this frame is stated here because it is the choke point every `$codex-bughunter:workflow-hunt` run passes through before any
...
box=greybox: creds already captured by `$codex-bughunter:workflow-hunt`, available in session memory.

**do not fan out across the authenticated hunt-\* set until the creds are
validated.** `$codex-bughunter:workflow-hunt` only prompts for and stores creds (skills/workflow-hunt/SKILL.md) — it
...
## step 4 — return control to $codex-bughunter:workflow-hunt

after taxonomy print, hand control back to `$codex-bughunter:workflow-hunt` for step 3 (sibling delegation) and step 4 (active testing). do not run probes here — this skill only loads context.

## privacy

never echo back, log, or persist:
- SOW / scope-of-work / engagement-letter content
- grey box credentials (kept in session memory by `$codex-bughunter:workflow-hunt`, never written to disk)
...
- **`hunt-rce`** / **`hunt-sqli`** / **`hunt-ssrf`** / **`hunt-ato`** / **all other hunt-* skills`** — When the mode-specific skill set is being printed. Workflow primitive: this skill is the loader; it names the hunt-* skills but does not run probes — actual hunting happens after step 4 returns control to `$codex-bughunter:workflow-hunt`.
