...
def _validate_pattern(pattern, label):
    """Reject malformed scope rules before they can weaken a deny list."""
    p = pattern.strip().lower()
    if p.startswith("re:"):
        if not p[3:]:
            raise ValueError(f"invalid {label} regex {pattern!r}: expression is empty")
        try:
            re.compile(p[3:])
        except re.error as error:
            raise ValueError(f"invalid {label} regex {pattern!r}: {error}") from error
        return
    if "/" in p:
        try:
            ipaddress.ip_network(p, strict=False)
        except ValueError as error:
            raise ValueError(f"invalid {label} CIDR {pattern!r}: {error}") from error
        return
    host = p[2:] if p.startswith("*.") else p
    if not host or (p.startswith("*.") and not "." in host):
        raise ValueError(f"invalid {label} host pattern {pattern!r}")
    try:
        ipaddress.ip_address(host)
        if p.startswith("*."):
            raise ValueError(f"invalid {label} wildcard IP pattern {pattern!r}")
        return
    except ValueError:
        if p.startswith("*.") and re.fullmatch(r"[0-9a-f:.]+", host):
            raise ValueError(f"invalid {label} wildcard IP pattern {pattern!r}")
    labels = host.rstrip(".").split(".")
    hostname_label = re.compile(r"[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?")
    if len(host) > 253 or not all(hostname_label.fullmatch(part) for part in labels):
        raise ValueError(f"invalid {label} host pattern {pattern!r}")
...
class Scope:
    def __init__(self, in_scope, out_of_scope=None, seeds=None, name="engagement"):
        ...
        for pattern in self.in_scope:
            _validate_pattern(pattern, "in-scope")
        for pattern in self.out_of_scope:
            _validate_pattern(pattern, "out-of-scope")
        self.seeds = seeds or []
        self.name = name
    ...
    def load(cls, path):
        import json
        with open(path, encoding="utf-8") as scope_file:
            d = json.load(scope_file)
...
def _selftest():
    s = Scope(in_scope=["example.com", "*.example.com", "*.test.example.com", "10.0.0.0/8",
                        "re:^lab[0-9]+\\.acme\\.io$"],
              out_of_scope=["admin.example.com", "internal.example.com"])
    ok = lambda t: s.in_scope_host(t)
    assert ok("https://example.com/login")           # apex
    assert ok("http://api.example.com/x")            # explicit wildcard
    ...
    s3 = Scope(in_scope=["api.example.com"])
    assert s3.in_scope_host("api.example.com")
    assert not s3.in_scope_host("nested.api.example.com")
    for bad in ("re:[", "999.0.0.0/8"):
        try:
            Scope(in_scope=["example.com"], out_of_scope=[bad])
            raise AssertionError(f"malformed scope pattern accepted: {bad}")
        except ValueError:
            pass
