...
(`offensive-osint` · scope-gated `cbh recon`) and the **hunt** phase
(`codex-bughunter` · `cbh surface` · `$codex-bughunter:workflow-hunt`).
...
| Producer | Fills |
|---|---|
| `cbh recon <target> --scope-file <scope.json>` | `target`, `generated_at`, `counts`, `assets`, `ranked_surface` |
...
| `scripts/hunt.* <target> [manifest.json]` | seeds `scope.md` (live hosts) + `notes.md` (ranked surface) |
| `$codex-bughunter:workflow-hunt`, `$codex-bughunter:workflow-surface` | same data, model-driven |
...
| `producers` | string[] | e.g. `["cbh-recon/2.1.1"]` |
...
```bash
cbh recon acme.com --scope-file ./scope.json # → recon/acme.com/manifest.json
cbh surface acme.com               # ranked P1/P2/Kill                 (consumer)
scripts/hunt.sh acme.com recon/acme.com/manifest.json # seeds scope.md + notes.md
```
