...
description: "Explicit Codex workflow. Run autonomous hunt loop on a target — scope check → recon → rank surface → hunt → validate → report with configurable checkpoints. Invoke only as $codex-bughunter:workflow-autopilot."
user-invocable: true
...
This workflow is explicit-only. Treat text after `$codex-bughunter:workflow-autopilot` as its arguments. Resolve bundled helpers relative to this installed skill (plugin root is two directories above `skills/workflow-autopilot`), while writing engagement artifacts only in the user's current workspace. Confirm authorization and enforce recorded scope before any active request.
...
```
$codex-bughunter:workflow-autopilot target.com                    # default: --paranoid mode
$codex-bughunter:workflow-autopilot target.com --normal           # batch checkpoint after validation
$codex-bughunter:workflow-autopilot target.com --yolo             # minimal checkpoints (still requires report approval)
$codex-bughunter:workflow-autopilot target.com --quick            # fast surface scan, fewer checks, lower token use
$codex-bughunter:workflow-autopilot targets.txt                   # multiple targets — one domain per line in the file
```

## Session Isolation (Important)

**Start a fresh Codex session per target.** Claude accumulates context across a session —
...
```bash
# Terminal 1: target A
codex   →  $codex-bughunter:workflow-autopilot targetA.com

# Terminal 2: target B (separate process)
codex   →  $codex-bughunter:workflow-autopilot targetB.com
```

If you must test multiple targets in one session, run `$codex-bughunter:workflow-pickup target.com` at the start of
...
```
$codex-bughunter:workflow-autopilot target.com --quick    # ~40% fewer tokens, covers main attack surface
$codex-bughunter:workflow-hunt target.com --vuln-class idor   # single bug class — lowest token use
```
...
- Capture is automatic; the ledger fills itself as you run. Manage size with `$codex-bughunter:workflow-memory-gc`.

## After Autopilot

- Run `$codex-bughunter:workflow-remember` to log successful patterns to hunt memory
- Run `$codex-bughunter:workflow-pickup target.com` next time to pick up where you left off
- Check the ledger (`~/.codex/bughunter/memory/findings.jsonl`) or run `$codex-bughunter:workflow-memory-gc` for its size
