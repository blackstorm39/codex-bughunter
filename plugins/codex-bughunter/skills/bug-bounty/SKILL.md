---
name: bug-bounty
description: "Complete authorized bug-bounty workflow covering recon, attack-surface ranking, web/API/LLM vulnerability hunting, bug chaining, validation, severity, PoC design, and platform-ready reporting. Use for a new in-scope target, source audit, specific vulnerability class, finding validation, or report drafting. Routes to focused hunt-* skills instead of loading the full corpus."
...
This skill ships inside the `codex-bughunter` plugin. Install the plugin from
its Codex marketplace; do not copy this file into a separate skill directory.
...
## Operator Notes (Codex-BugHunter)
...
The explicit `$codex-bughunter:workflow-hunt` skill and the `hunt <target>` shell helper pre-create the engagement scaffold:

- `targets/<target>$codex-bughunter:workflow-scope.md` — declared scope, pasted from the program page
...
- `targets/<target>$codex-bughunter:workflow-recon/` — outputs from `subfinder | dnsx | httpx | katana`
