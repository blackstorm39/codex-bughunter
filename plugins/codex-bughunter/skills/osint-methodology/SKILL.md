...
description: "Comprehensive OSINT methodology for authorized external red-team and attack-surface assessments. Covers staged recon, asset graphs, confidence and severity, identity fabrics, APIs, JavaScript, mobile and cloud surfaces, breach correlation, detection-aware probing, CVE prioritization, reporting, threat-actor investigation, crypto tracing, media forensics, and chronolocation. Use for scoped organizational mapping or evidence-led investigations."
...
Run these prompts in a fresh Codex task to verify the skill loads and behaves correctly. Pass criteria: expected sections referenced, no hallucinated content, scope-check invoked when needed.
