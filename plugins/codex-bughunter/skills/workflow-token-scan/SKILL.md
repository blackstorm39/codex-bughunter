---
name: workflow-token-scan
description: "Explicit Codex workflow. Meme coin and token security scan — checks for rug pull vectors (hidden mint, honeypot, fee manipulation, LP lock bypass, authority retention, bonding curve exploits, fake renounce, sandwich amplification). Manual 8-class grep audit (with an optional automated scanner if present). Invoke only as $codex-bughunter:workflow-token-scan."
user-invocable: true
...
This workflow is explicit-only. Treat text after `$codex-bughunter:workflow-token-scan` as its arguments. Resolve bundled helpers relative to this installed skill (plugin root is two directories above `skills/workflow-token-scan`), while writing engagement artifacts only in the user's current workspace. Confirm authorization and enforce recorded scope before any active request.
...
```
$codex-bughunter:workflow-token-scan contracts/Token.sol                          # Single EVM contract
$codex-bughunter:workflow-token-scan src/ --recursive                             # Scan entire directory
$codex-bughunter:workflow-token-scan programs/token/ --chain solana --recursive   # Solana program
$codex-bughunter:workflow-token-scan contracts/Token.sol --output findings/report.md  # Save report
```
