...
`codex-bughunter` is a Codex plugin and knowledge bundle. It contains methodology, payloads, bypass tables, detection patterns, and reporting templates derived from publicly disclosed bug-bounty reports and authorized engagements.
...
You are installing 98 `SKILL.md` files plus shell and Python helpers into your AI agent's context. Agent Skills are third-party code — treat them like any dependency you run. Independent research (Snyk "ToxicSkills", 2026) found prompt injection in a meaningful fraction of public skills, so verification matters.

**What we do on our side:**

- **CI gate on every skill change** — the root Gitea workflow runs skill lint, unit tests, syntax checks, and package build checks.
- **Reviewable provenance** — the local marketplace records the plugin source, version, and repository. Pin a reviewed checkout yourself with `git checkout <tag-or-commit>` before installing.
- **Narrow installer behavior** — the installer invokes Codex plugin marketplace/install commands for local files. It does not edit shell startup files or PowerShell profiles. Optional Burp MCP setup runs only when an explicit URL or JAR is supplied.
...
- **Skill content** that could enable abuse against unauthorized targets without commensurate defensive value: report it privately to the local Gitea project owner.
- **Vulnerabilities in installer scripts** (`scripts/install.sh`, `scripts/install.ps1`, engagement scaffolds, or MCP setup): use the same channel.
...
1. **Validate first** — invoke `$codex-bughunter:workflow-triage` or `$codex-bughunter:workflow-validate`.
...
By using `codex-bughunter`, you acknowledge:

- You are responsible for ensuring you have authorization to test any target you give Codex
