#!/usr/bin/env python3
"""
cbh — Codex BugHunter CLI.

Bridges the repo's skill content into a real runner. Five subcommands compose
the engagement loop:

  cbh recon <target>           scope-gated passive subdomain enum + live-host probe + URL
...
Or run inline:
    ...
    scripts/cbh.py recon target.com --scope-file scope.json
"""
...
CLONE_MODE = SKILLS_DIR.is_dir() and any(SKILLS_DIR.iterdir())
REPO_URL = "https://git.ot9.me/Adminx/codex-bughunter"
...
def say(s: str = ""):
    ...
    printable = str(s).encode(encoding, errors="replace").decode(encoding)
    print(printable)
...
class _NoRedirectHandler(urllib.request.HTTPRedirectHandler):
    """Return redirect responses without contacting an unvalidated destination."""

    def redirect_request(self, req, fp, code, msg, headers, newurl):
        return None


_DIRECT_OPENER = urllib.request.build_opener(_NoRedirectHandler())
...
def configure_http_proxy(proxy_url: str | None = None, insecure: bool = False) -> tuple[bool, str]:
    ...
    _HTTP_OPENER = urllib.request.build_opener(proxy_handler, https_handler, _NoRedirectHandler())
...
def detect_burp() -> str | None:
    ...
    try:
        with _DIRECT_OPENER.open("http://127.0.0.1:8080/", timeout=1) as r:
...
def http_get(url: str, timeout: int = 5, headers: dict | None = None) -> tuple[int, dict, str]:
    ...
    try:
        ...
        try:
            ...
            e.close()
        return e.code, response_headers, body
...
def cmd_recon(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"[error] cannot load scope file: {error}", file=sys.stderr)
        return 1
    reason = scope.reject_reason(target)
    if reason:
        print(f"[error] target is out of scope: {reason}", file=sys.stderr)
        return 1
    ...
    subs = {host for host in subs if scope.in_scope_host(host)}
    excluded_count = discovered_count - len(subs)
    if excluded_count:
        say(f"  Scope filter: excluded {excluded_count} out-of-scope candidate(s)")
    ...
    say()
    say(f"  Next: {color('cbh classify <url>', 'bold')} for fast pattern-match, or "
        f"{color('$codex-bughunter:workflow-hunt <target>', 'bold')} in Codex")
    return 0
...
def write_recon_summary(target: str, subs: set[str], resolved: dict, live: list, out: Path):
    lines = [f"# Recon — {target}",
             "",
             f"_Generated by scope-gated `cbh recon {target}` at "
...
def cmd_classify(args: argparse.Namespace) -> int:
    ...
    if not result["matches"]:
        ...
        say(f"    {color('cbh recon <target> --scope-file <scope.json>', 'bold')} "
            "to map attack surface first.")
        return 1
    say()
    for skill, reasons in sorted(result["matches"].items()):
        ...
        if local_report.exists():
            ...
            say(f"      📖 Pattern Library: {color(f'{REPO_URL}/src/branch/main/plugins/codex-bughunter/docs/disclosed-reports/{report_name}', 'cyan')}")
        skill_path = SKILLS_DIR / skill / "SKILL.md"
        if skill_path.exists():
            say(f"      📋 Skill:           {color(str(skill_path), 'cyan')}")
        elif not CLONE_MODE:
            say(f"      📋 Skill:           {color(f'{REPO_URL}/src/branch/main/plugins/codex-bughunter/skills/{skill}/SKILL.md', 'cyan')}")
        say()
    ...
    say()
    say(color("Need richer context? In Codex:", "dim"))
    say(color("   $codex-bughunter:workflow-hunt <url> — full routing with model judgment", "dim"))
...
def cmd_surface(args: argparse.Namespace) -> int:
    ...
    if not mpath or not mpath.exists():
        print(f"[error] no recon manifest for {target}. Run:  cbh recon {target} "
              "--scope-file <scope.json>", file=sys.stderr)
        return 1
    try:
    ...
    say()
    say(f"  Next: {color('$codex-bughunter:workflow-hunt ' + target, 'bold')} in Codex, "
        f"or {color('cbh classify <url>', 'bold')} for a single URL")
    return 0


# ============================================================
# Codex engagement helpers — portable engine entry points
# ============================================================
def cmd_init(args: argparse.Namespace) -> int:
    destination = Path(args.dir or args.name).expanduser().resolve()
    scope_path = destination / "scope.json"
    agents_path = destination / "AGENTS.md"
    if scope_path.exists() or agents_path.exists():
        print(f"[error] refusing to overwrite an existing engagement in {destination}", file=sys.stderr)
        return 1
    destination.mkdir(parents=True, exist_ok=True)
    in_scope = args.in_scope or [args.name]
    seeds = args.seed or [f"https://{in_scope[0].lstrip('*.')}"]
    scope = {
        "name": args.name,
        "in_scope": in_scope,
    ...
    agents_path.write_text(
        f"# Engagement: {args.name}\n\n"
        "Only test assets explicitly authorized in `scope.json`. Deny rules win and "
        "unknown assets are out of scope. Start with "
        "`$codex-bughunter:workflow-scope`, then `$codex-bughunter:workflow-recon`.\n",
        encoding="utf-8",
    )
    say(f"Initialized {destination}")
    return 0
...
def cmd_scope_check(args: argparse.Namespace) -> int:
    ...
    for target in args.targets:
        reason = scope.reject_reason(target)
        if reason:
            say(f"OUT-OF-SCOPE {target} — {reason}")
            failed = True
        else:
            say(f"IN-SCOPE     {target}")
    return 1 if failed else 0


def cmd_autopilot(args: argparse.Namespace) -> int:
    from engine.engine import ALL_PHASES, DEFAULT_PHASES, Engine

    phases = [part.strip() for part in args.phases.split(",") if part.strip()] if args.phases else (
        ALL_PHASES if args.hunt else DEFAULT_PHASES
    )
    runner = Engine(
        args.scope_file, args.base, args.model, args.max_hunts, args.max_turns,
        args.timeout, args.mock, args.allow_intrusive, args.parallel,
        args.expand, args.use_memory,
    )
    runner.run(phases)
    say(json.dumps(runner.eng.summary(), indent=2))
    say(f"engagement dir: {runner.eng.dir}")
    return 0
...
def cmd_memory(args: argparse.Namespace) -> int:
    ...
    if args.host:
        result = memory.rollup(args.host)
        if result is None:
            print(f"[error] no memory rollup for {args.host}", file=sys.stderr)
            return 1
        say(json.dumps(result, indent=2))
        return 0
    action = {"report": "report", "rotate": "rotate", "purge": "purge-backups"}[args.action]
    say(json.dumps(memory.gc(action=action, max_mb=args.max_mb, root=args.dir), indent=2))
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="cbh",
        description=("Codex BugHunter CLI — terminal-native deterministic runner.\n"
                     "Use plugin workflow skills for model-driven work; use cbh for CI/CD, "
                     "scripted runs, and deterministic verification."),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent("""\
            Examples:
              cbh recon hackerone.com --scope-file scope.json
              cbh recon target.com --scope-file scope.json --burp  # route via Burp proxy
            ...
            For model-driven hunting with full skill context, invoke plugin workflows
            explicitly in Codex, for example: $codex-bughunter:workflow-hunt <target>
    ...
    p_recon.add_argument("--scope-file", required=True,
                         help="written authorization scope JSON (deny wins, default deny)")
    ...
    p_init = sub.add_parser("init", help="create a Codex engagement with scope.json and AGENTS.md")
    p_init.add_argument("name", help="engagement name or default in-scope domain")
    p_init.add_argument("--dir", help="destination directory (default: ./<name>)")
    p_init.add_argument("--in-scope", action="append", default=[], metavar="PATTERN",
                        help="exact host/CIDR/regex; use *.example.com for descendants")
    p_init.add_argument("--out-of-scope", action="append", default=[], metavar="PATTERN",
                        help="exact host/CIDR/regex deny rule (deny wins)")
    p_init.add_argument("--seed", action="append", default=[], metavar="URL")
    p_init.set_defaults(func=cmd_init)

    p_scope = sub.add_parser("scope", help="deterministically check targets against scope.json")
    p_scope.add_argument("targets", nargs="+", help="hosts or URLs to check")
    p_scope.add_argument("--scope-file", required=True, help="engagement scope.json")
    p_scope.set_defaults(func=cmd_scope_check)

    p_auto = sub.add_parser("autopilot", help="run the portable engagement engine")
    p_auto.add_argument("--scope-file", required=True, help="engagement scope.json")
    p_auto.add_argument("--base", default="~/.codex/bughunter/engagements")
    p_auto.add_argument("--phases", help="comma-separated engine phases")
    p_auto.add_argument("--hunt", action="store_true", help="opt in to model-driven hunt and validation")
    p_auto.add_argument("--model", help="optional Codex model override")
    p_auto.add_argument("--max-hunts", type=int, default=10)
    p_auto.add_argument("--max-turns", type=int, default=40, help=argparse.SUPPRESS)
    p_auto.add_argument("--timeout", type=int, default=600)
    p_auto.add_argument("--mock", action="store_true")
    p_auto.add_argument("--allow-intrusive", action="store_true")
    p_auto.add_argument("--parallel", type=int, default=3)
    p_auto.add_argument("--expand", action="store_true")
    p_auto.add_argument("--use-memory", action="store_true")
    p_auto.set_defaults(func=cmd_autopilot)

    p_memory = sub.add_parser("memory", help="inspect or maintain Codex BugHunter memory")
    p_memory.add_argument("--host", help="show one target rollup")
    p_memory.add_argument("--action", choices=["report", "rotate", "purge"], default="report")
    p_memory.add_argument("--max-mb", type=int, default=10)
    p_memory.add_argument("--dir", help="override memory directory")
